...
public protocol HyperLinkOperationDelegate: AnyObject {
    func doesContainUser(userName: String, senderID: String) -> Bool
}
...
public typealias ConfigureLinkAttribute = (HyperlinkType, [NSAttributedString.Key : Any], Bool) -> ([NSAttributedString.Key : Any])
typealias ElementTuple = (range: NSRange, element: HyperlinkElement, type: HyperlinkType)

@IBDesignable open class HyperlinkLabel: UILabel {
    
    // MARK: - public properties
    open weak var delegate: HyperlinkLabelDelegate?
    
    open weak var hyperLinkOperationDelegate: HyperLinkOperationDelegate?
    
    open var enabledTypes: [HyperlinkType] = [.mention, .hashtag, .url]
    
    open var urlMaximumLength: Int?
    
    open var configureLinkAttribute: ConfigureLinkAttribute?
    
    @IBInspectable open var mentionColor: UIColor = .getAppColor(colorName: .purple_5B2AC5) {
    ...
    open var customColor: [HyperlinkType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    open var customSelectedColor: [HyperlinkType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleCustomTap(for type: HyperlinkType, handler: @escaping (String) -> ()) {
        customTapHandlers[type] = handler
    }
    
    open func removeHandle(for type: HyperlinkType) {
        switch type {
        case .hashtag:
    ...
    open var senderID: String = .blank
...
//        let range = NSRange(location: 0, length: textStorage.length + 100)
    ...
    // MARK: - customzation
    @discardableResult
    open func customize(_ block: (_ label: HyperlinkLabel) -> ()) -> HyperlinkLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    internal var customTapHandlers: [HyperlinkType : ((String) -> ())] = [:]
    ...
    lazy var hyperlinkElements = [HyperlinkType: [ElementTuple]]()
    ...
    fileprivate func clearHyperlinkElements() {
        selectedElement = nil
        for (type, _) in hyperlinkElements {
            hyperlinkElements[type]?.removeAll()
        }
    }
...
//        attributes[NSAttributedString.Key.foregroundColor] = mentionColor
    ...
    private func filterPresentNamesOnMention() {
        guard let mentions = hyperlinkElements[.mention] else { return }
        let filteredMentions = mentions.filter({ range, element, type in
            switch element {
            case .mention(let mentionName):
                return hyperLinkOperationDelegate?.doesContainUser(userName: mentionName, senderID: self.senderID) ?? false
            default:
                return false
            }
        })
        hyperlinkElements[.mention] = filteredMentions
    }
    
    /// use regex check all link ranges
    fileprivate func parseTextAndExtractHyperlinkElements(_ attrString: NSAttributedString) -> String {
        var textString = attrString.string
        var textLength = textString.utf16.count
        var textRange = NSRange(location: 0, length: textLength)
        
        if enabledTypes.contains(.url) {
            let tuple = HyperlinkBuilder.createURLElements(from: textString, range: textRange, maximumLength: urlMaximumLength)
            ...
            hyperlinkElements[.url] = urlElements
        }
        ...
        for type in enabledTypes where type != .url {
            ...
            let hashtagElements = HyperlinkBuilder.createElements(type: type, from: textString, range: textRange, filterPredicate: filter)
            hyperlinkElements[type] = hashtagElements
        }
...
extension HyperlinkLabel: UIGestureRecognizerDelegate {
