 ...
 class MessageActions: UITableViewController, PanModalPresentable, CreateMeetingDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let currentActions = actions {
            ...
            case .pinMessage:
                if let cell = tableView.dequeueReusableCell(withIdentifier: "actionsCell", for: indexPath) as? ActionsCell {
                    cell.name.text = "PIN_MESSAGE".localized()
                    cell.icon.image = UIImage.getAppImage(imageName: .pin_icon).withRenderingMode(.alwaysTemplate)
                    if #available(iOS 13.0, *) {
                        cell.name.textColor = .label
                    }else{
                        cell.name.textColor = .black
            ...
            case .unpinMessage:
                if let cell = tableView.dequeueReusableCell(withIdentifier: "actionsCell", for: indexPath) as? ActionsCell {
                    cell.name.text = "UNPIN_MESSAGE".localized()
                    cell.icon.image = UIImage.getAppImage(imageName: .unpin_icon).withRenderingMode(.alwaysTemplate)
                    if #available(iOS 13.0, *) {
                        cell.name.textColor = .label
                    }else{
                        cell.name.textColor = .black
            ...
            case .sendGifs:
                if let cell = tableView.dequeueReusableCell(withIdentifier: "actionsCell", for: indexPath) as? ActionsCell {
                    cell.name.text = "SEND_GIFS".localized()
                    cell.icon.image = UIImage(named: "messages-stickers.png", in: UIKitSettings.bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
                    cell.fullScreenSwitch.isHidden = true
                    cell.badgeCountSwitch.isHidden = true
                    cell.icon.tintColor = UIKitSettings.secondaryColor
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if let currentActions = actions {
            ...
            case .whiteboard, .writeboard, .messageTranslate, .sticker, .share, .thread, .delete, .forward, .edit, .reply, .copy ,.messageInfo ,.createAPoll ,.shareLocation ,.document , .photoAndVideoLibrary, .takeAPhoto, .createMeeting, .messageInPrivate, .replyInPrivate, .pinMessage, .unpinMessage, .sendGifs:  return 55.0
            }
        }else{
            return 0
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let currentActions = actions {
            ...
            case .pinMessage:
                self.dismiss(animated: true) {
                    MessageActions.actionsDelegate?.didPinMessagePressed()
                }
            case .unpinMessage:
                self.dismiss(animated: true) {
                    MessageActions.actionsDelegate?.didUnpinMessagePressed()
                }
            case .sendGifs:
                self.dismiss(animated: true){
                    MessageActions.actionsDelegate?.didSendGifsPressed()
                }
            }
        }
    }
