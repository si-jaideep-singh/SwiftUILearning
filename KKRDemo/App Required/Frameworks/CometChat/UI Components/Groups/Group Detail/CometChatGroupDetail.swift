                                ...
                                CometChat.banGroupMember(UID: selectedCell.member?.uid ?? "", GUID: self.currentGroup?.guid ?? "", onSuccess: { (success) in
                                    DispatchQueue.main.async {
                                        if let group = self.currentGroup {
                                            ...
                                            group.removeHigherAccess(userID: selectedCell.member?.uid)
                                            CommonUtilities.shared.updateCometChatGroupMetaData(group: group)
                                        }
                    ...
                    present(alert, animated: true, completion: nil)
                        ...
                        alert.addAction(UIAlertAction(title: "LEAVE".localized(), style: .default, handler: { (action: UIAlertAction!) in
                            if let guid = self.currentGroup?.guid {
                                ...
                                let group = self.currentGroup!

                                // Updating group meta data to remove user from admin or mods
                                group.removeHigherAccess(userID: user.uid)
                                CommonUtilities.shared.updateCometChatGroupMetaData(group: group) { newGroup in
                                    
                                    // on success removing user from comet chat group
                                    CometChat.leaveGroup(GUID: guid, onSuccess: { (success) in
                                        self.updateUserMetaData(clubId: guid, user: user, metaDataEvent: .groups, event: .remove) {
                    ...
                    CometChat.kickGroupMember(UID: selectedCell.member?.uid ?? "", GUID: self.currentGroup?.guid ?? "", onSuccess: { (success) in
                        DispatchQueue.main.async {
                            if let group = self.currentGroup {
                                ...
                                CometChat.getUser(UID: selectedCell.member?.uid ?? "") { user in
                                    self.updateUserMetaData(clubId: group.guid , user: user!, metaDataEvent: .kicked, event: .add) {
                        ...
                        DispatchQueue.main.async {
                            if let group = self.currentGroup {
                                ...
                                CometChat.getUser(UID: selectedCell.member?.uid ?? "") { user in
                                    ...
                                    self.updateUserMetaData(clubId: group.guid, user: user, metaDataEvent: .banned, event: .add) {
    ...
    public func onGroupMemberLeft(action: ActionMessage, leftUser: CometChatPro.User, leftGroup: CometChatPro.Group) {
        if let group = currentGroup {
            if group == leftGroup {
                ...
                updateUserMetaData(clubId: group.guid, user: leftUser, metaDataEvent: .groups, event: .remove) {
                    print("Success")
                } onFailure: { error in
...
//        if let userData = leftUser.metadata as? FanClubUserMetaData {
//        }
    }
    
    private func performClubIDEventToUser(clubID: String, user: CometChatPro.User, metaDataKey: FanClubMetaDataKeys, event: FanClubUserMetaDataEvent) -> CometChatPro.User {
        var user = user
        var metaData = user.metadata
        var groups = metaData?[metaDataKey.name] as? [String]
        
        switch event {
        case .add:
            if metaDataKey != .groups {
                var joinedGroups = metaData?[FanClubMetaDataKeys.groups.name] as? [String]
                joinedGroups?.removeAll(where: { value in
                    value == clubID
                })
                metaData?[FanClubMetaDataKeys.groups.name] = joinedGroups
            }
            groups?.append(clubID)
        case .remove:
            groups?.removeAll(where: { value in
                value == clubID
            })
//            if metaDataKey != .groups {
//                var joinedGroups = metaData?[FanClubMetaDataKeys.groups.name] as? [String]
//                joinedGroups?.append(clubID)
//                metaData?[FanClubMetaDataKeys.groups.name] = joinedGroups
//            }
        }

        metaData?[metaDataKey.name] = groups
        user.metadata = metaData
        return user
    }
    
    func updateUserMetaData(clubId: String,
                            user: CometChatPro.User,
                            metaDataEvent: FanClubMetaDataKeys,
                            event: FanClubUserMetaDataEvent,
                            onSuccess: @escaping() -> Void,
                            onFailure: @escaping(CometChatException) -> Void) {
        
//        user.tags.append(clubId)
        let updatedUser = performClubIDEventToUser(clubID: clubId, user: user, metaDataKey: metaDataEvent, event: event)
        CometChat.updateUser(user: updatedUser, apiKey: Constants.KKRAppConstants.cometChatCred?.cometChatAuthKey ?? .blank) { (user) in
                onSuccess()
        } onError: { (error) in
            if let error = error {
                onFailure(error)
...
//                updateUserMetaData(clubId: group.guid, user: kickedUser, metaDataEvent: .kicked, event: .add) {
//                    print("Success")
//                } onFailure: { error in
//                    print(error.errorDescription)
//                }
...
//                updateUserMetaData(clubId: group.guid, user: bannedUser, metaDataEvent: .banned, event: .add) {
//                    print("Success")
//                } onFailure: { error in
//                    print(error.errorDescription)
//                }
...
//                updateUserMetaData(clubId: group.guid, user: unbannedUser, metaDataEvent: .banned, event: .remove) {
//                    print("Success")
//                } onFailure: { error in
//                    print(error.errorDescription)
//                }
