        ...
        didSet {
            if let currentGroup = group {
               ...
               /// Set the group icon
                ...
                if let groupIcon = currentGroup.icon,  !groupIcon.isEmpty {
                    ...
                    if let defaultFCGroupAvatar = FirebaseModelConfig.sharedFirebaseConfig.appStaticImages?.defaultFCGroupAvatar {
                        icon.set(image: defaultFCGroupAvatar)
                    }
                    // icon.image = .getAppImage(imageName: .fanChatDefaultGroupIcon)
                    icon.backgroundColor = .getAppColor(colorName: .purple_3B215D)
                }
                if #available(iOS 13.0, *) {
                    let videoCallIcon = UIImage(named: "videoCall.png", in: UIKitSettings.bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
                    videoCall.setImage(videoCallIcon, for: .normal)
